...
public class SwipeRefreshLoadLayout extends ViewGroup {
    ...
    public SwipeRefreshLoadLayout(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.SwipyRefreshLoadLayout);
        SwipeDirection direction = SwipeDirection.getFromInt(a2.getInt(R.styleable.SwipyRefreshLoadLayout_direction, 0));
        if (direction != SwipeDirection.BOTH) {
            mDirection = direction;
            mBothDirection = false;
        } else {
            mDirection = SwipeDirection.TOP;
            mBothDirection = true;
        }
        a2.recycle();
...
//    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    if (y > mInitialMotionY) {
                        setRawDirection(SwipeDirection.TOP);
                    } else if (y < mInitialMotionY) {
                        setRawDirection(SwipeDirection.BOTTOM);
                    }
                    if ((mDirection == SwipeDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == SwipeDirection.TOP && canChildScrollUp())) {
                        return false;
                    }
                }
                float yDiff;
    ...
    public SwipeDirection getDirection() {
        return mBothDirection ? SwipeDirection.BOTH : mDirection;
    }
    ...
    // only TOP or Bottom
    private void setRawDirection(SwipeDirection direction) {
